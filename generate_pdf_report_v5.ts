...
async function generateReport() {
  try {
    ...
    records.forEach((r: any) => {
      ...
      // Support for potential Date objects from Firebase
      ...
      const minutes = calculateMinutes(r.inTime, r.outTime);
    ...
    const multiple = Object.entries(sewadarData)
      ...
      .map(([name, groups]) => {
        ...
        const totalOverallRecords = Object.values(groups).reduce((acc, curr) => acc + curr.recordCount, 0);
      ...
      .sort((a, b) => b.groupCount - a.groupCount || b.totalRecords - a.totalRecords);
    ...
    doc.text("Breakdown of volunteering days and sewa hours per group for Gents Sewadars appearing in multiple groups.", 14, currentY);
    ...
    doc.text("Analysis Summary", 14, currentY);
    ...
    currentY += 5;
    doc.text(`Report generated on: ${new Date().toLocaleDateString('en-GB')}`, 14, currentY);
    ...
    (doc as any).autoTable({
      startY: currentY + 10,
      head: [['S.No', 'Sewadar Name', 'Groups', 'Detailed Breakdown (Group: Days & Hours)', 'Total Records']],
      body: tableData,
    ...
    console.log("PDF Report updated with detailed breakdown: Multiple_Group_Attendance_Report.pdf");
  } catch (error) {
