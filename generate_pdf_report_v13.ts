...
async function generateReport() {
  try {
    ...
    // 1. Calculate active duty days (sessions) for each group in total history
    ...
    const tableData = reportList.map((s, index) => {
      const breakdownText = s.breakdown.length > 0 
        ? s.breakdown.map(b => {
            const totalGroupDays = groupActiveDates[b.name]?.size || 0;
            return `${b.name}: ${b.days}/${totalGroupDays} days (${b.hours})`;
          }).join('\n')
    ...
    (doc as any).autoTable({
      startY: currentY + 10,
      head: [['S.No', 'Sewadar Name', 'Groups', 'Detailed Breakdown (Group: Present/Total Duty Days & Hours)', 'Total Days & Hours']],
      body: tableData,
