...
async function listWednesday() {
  const q = query(collection(db, 'attendance'), where('group', '==', 'Wednesday'));
  const snap = await getDocs(q);
  const names = Array.from(new Set(snap.docs.map(d => d.data().name)));
  console.log("Names in Wednesday group:");
  names.sort().forEach(n => console.log(`- ${n}`));
}

listWednesday().then(() => process.exit(0));
