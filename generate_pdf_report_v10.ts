...
// whistlelist of names provided by the user
...
async function generateReport() {
  try {
    ...
    console.log(`Analyzing ${allRecords.length} records for targeted list...`);
    ...
    const sewadarDisplayNames: Record<string, string> = {}; // Keep the best display name
    
    // Track which norm names from the whitelist were actually found in multi-groups
    ...
    validGroupRecords.forEach((r: any) => {
      ...
      if (!sewadarData[normName]) {
        ...
        // Determine display name: use user-provided whitelist version if possible, otherwise raw
        const userProvided = whitelistedNamesRaw.find(un => normalizeName(un) === normName);
        sewadarDisplayNames[normName] = userProvided || rawName;
      }
    ...
    const finalData = Object.entries(sewadarData)
      ...
      // The user wants a report of these people specifically. 
      // Usually "Multiple Group Report" implies we only show those with count > 1.
      // However, if the user explicitly asked for these 41, I will show those from the list
      // who actually have attendance records, prioritized by multi-group status.
    ...
    const introText = `With the blessings of H.H. Sant Rajinder Singh Ji Maharaj, Targeted Gents Security Group Attendance Analysis Report.`;
    ...
    doc.text("Restricted to specific sewadars as requested (merging name variations).", 14, currentY);
