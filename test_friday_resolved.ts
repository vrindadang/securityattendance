...
async function computeAveragesResolved() {
  ...
  const logicalGroups = ['Monday Gents', 'Tuesday Gents', 'Wednesday Gents', 'Thursday Gents', 'Friday Gents', 'Saturday Gents', 'Sunday Gents'];
  const locationsList = ['Kirpal Bagh', 'Kirpal Ashram', 'Sawan Ashram', 'Sant Darshan Singh Ji Dham'];

  // Map of Map: normName -> Map of group -> sewadar
  const nameAndGroupLookup = new Map<string, Map<string, any>>();
  const idLookup = new Map();
  const nameLookup = new Map(); // default fallback
  ...
  mergedRoster.forEach(s => {
    ...
    if (!nameAndGroupLookup.has(norm)) {
      nameAndGroupLookup.set(norm, new Map());
    }
    nameAndGroupLookup.get(norm)!.set(s.group, s);
  });
  ...
  let totalMappedToFridayGents = 0;
  ...
  rawAttendance.forEach(r => {
    ...
    // Resolve matchedSewadar
    let matchedSewadar = sId ? idLookup.get(String(sId)) : null;
    if (!matchedSewadar) {
      // Disambiguate with record group
      const recordDay = rawGroup.replace(' Gents', '').replace(' Ladies', '').replace('Ladies-', '').trim();
      const groupSubmap = nameAndGroupLookup.get(normName);
      if (groupSubmap && groupSubmap.has(recordDay)) {
        matchedSewadar = groupSubmap.get(recordDay);
      } else {
        matchedSewadar = nameLookup.get(normName);
      }
    }
  ...
  console.log(`Resolved mapping: total mapped to Friday Gents: ${totalMappedToFridayGents}`);

  locationsList.forEach(loc => {
    console.log(`\n=== LOCATION: ${loc} ===`);
    const g = 'Friday Gents';
...
computeAveragesResolved().then(() => process.exit(0));
