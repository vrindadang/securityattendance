...
function normalizeName(name: string): string {
  ...
  n = n.replace(/^DR\s+/g, '');
  n = n.replace(/^MR\s+/g, '');
  // Remove all non-alphabetical characters to merge dots/spaces/brackets
  n = n.replace(/[^A-Z]/g, '');
  return n;
}
...
async function generateReport() {
  try {
    console.log("Fetching all historical attendance records for Gents...");
    const q = query(collection(db, 'attendance'), where('gender', '==', 'Gents'));
    const snapshot = await getDocs(q);
    const allRecords = snapshot.docs.map(doc => doc.data());
    
    console.log(`Analyzing ${allRecords.length} records...`);

    // Filter out 'Global' as per user instruction "ignore global"
    ...
    const sewadarDisplayNames: Record<string, Set<string>> = {};
    ...
    records.forEach((r: any) => {
      ...
      if (!sewadarData[normName]) {
        ...
        sewadarDisplayNames[normName] = new Set();
      }
      ...
      sewadarDisplayNames[normName].add(rawName);
    });
    ...
    const multiple = Object.entries(sewadarData)
      ...
      .map(([normName, groups]) => {
        ...
        // Pick the most complete name as the display name (longest string usually covers surnames/details)
        const nameList = Array.from(sewadarDisplayNames[normName]).sort((a, b) => b.length - a.length);
        const displayName = nameList[0];
      ...
      .sort((a, b) => b.groupCount - a.groupCount || b.totalDays - a.totalDays);
    ...
    doc.text("Consolidated historical data using robust name matching (merging variations like D.L. Kapoor / DL Kapoor).", 14, currentY);
    ...
    currentY += 5;
    doc.text(`Analysis period: Since App Inception to ${new Date().toLocaleString('en-GB')}`, 14, currentY);
    ...
    (doc as any).autoTable({
      startY: currentY + 10,
      head: [['S.No', 'Sewadar Name', 'Groups', 'Detailed Breakdown (Group: Day & Hour Summary)', 'Total Days & Hours']],
      body: tableData,
    ...
    console.log("PDF Report updated successfully: Multiple_Group_Attendance_Report.pdf");
  } catch (error) {
